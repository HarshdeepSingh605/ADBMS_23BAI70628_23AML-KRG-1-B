...
CREATE OR REPLACE PROCEDURE P_GENDER_DIVERSITY( IN GEND VARCHAR(10), OUT TOTAL_NUMBER INT)
LANGUAGE plpgsql AS 
$$
BEGIN
	SELECT COUNT(ID) INTO TOTAL_NUMBER
	FROM EMPLOYEE_INFO
	WHERE GENDER=GEND;
	RAISE NOTICE 'NUMBER OF % EMPLOYEE ARE : %',GEND,TOTAL_NUMBER;
END;
$$;

CALL P_GENDER_DIVERSITY('Male',NULL)
