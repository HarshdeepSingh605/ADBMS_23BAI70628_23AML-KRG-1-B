...
SELECT * FROM V_NORMAL


-- CREATE A MATERIALISED VIEW
CREATE MATERIALIZED VIEW V_MATERIALISED AS 
	SELECT COUNT(*) AS TOTAL_ORDERS,
	SUM(VALUE) AS TOTAL_SALES,
...
CREATE OR REPLACE VIEW V_ORDER_SUMMARY AS 
	SELECT 
		...
		P.PRODUCT_NAME,
		C.FULL_NAME,
		O.QUANTITY*(P.UNIT_PRICE - (P.UNIT_PRICE*(O.DISCOUNT_PERCENT/100))) AS FINAL_PRICE
...
GRANT SELECT  ON V_ORDER_SUMMARY TO ROHIT

REVOKE SELECT ON V_ORDER_SUMMARY FROM ROHIT
